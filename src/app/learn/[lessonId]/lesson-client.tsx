...
// ── Phil, your lesson buddy ─────────────────────────────────────────────────
// A little reacting Photo Phil + speech bubble that follows along with the
// lesson. He reads while you learn, thinks on a question, cheers when you
// nail it, and gasps when you miss — so the learning never feels lonely.

type BuddyMood = { pose: PhilPose; line: string };

const READING_LINES = [
  "Ooh, this is a good one. Soak it in!",
  "Let's frame up something beautiful.",
  "Every pro started right here. Let's go!",
  "Little tip, big difference. Watch this.",
];
...
function getBuddyMood(
  ...
  if (stepType === 'quiz') {
    if (!answered) return { pose: 'thinking', line: "Take your time… what do you think?" };
    if (wasCorrect) {
      return isPerfect
        ? { pose: 'starstruck', line: "Flawless! You've got the eye for this. 🌟" }
        : { pose: 'cheer', line: "Yes! Nailed it. 🎉" };
    }
    return { pose: 'surprised', line: "So close! Now you'll never forget it." };
  }
  if (stepType === 'when-to-use') {
    return { pose: 'excited', line: "Here's when to pull this trick out. 👀" };
  }
  return { pose: 'reading', line: pickLine(READING_LINES, stepIdx) };
}

function LessonBuddy({ mood }: { mood: BuddyMood }) {
  return (
    <div className="flex items-end gap-2.5 mb-4 px-1">
      <div className="flex-shrink-0 animate-bob" style={{ filter: 'drop-shadow(0 6px 10px rgba(0,0,0,0.10))' }}>
        <PhotoPhil size={68} pose={mood.pose} />
      </div>
      <div
        key={mood.line}
        className="animate-pop relative mb-2 rounded-2xl px-3.5 py-2.5"
        style={{
          background: 'var(--surface)',
          border: '1.5px solid var(--border)',
          boxShadow: '0 3px 10px rgba(27,154,228,0.10)',
        }}
      >
        {/* little tail pointing back at Phil */}
        <span
          style={{
            position: 'absolute', left: -6, bottom: 12, width: 12, height: 12,
            background: 'var(--surface)', borderLeft: '1.5px solid var(--border)',
            borderBottom: '1.5px solid var(--border)', transform: 'rotate(45deg)',
          }}
        />
        <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--foreground)', lineHeight: 1.35 }}>
          {mood.line}
        </p>
      </div>
    </div>
...
export default function LessonClient({ lessonId }: { lessonId: string }) {
  ...
  const wasCorrect = answered && selected === step.correct;
  const buddyMood = getBuddyMood(step.type, stepIdx, answered, wasCorrect, wasCorrect && !missedAny);
          ...
          <div className="flex-1 flex flex-col justify-between">
            <div>
              {step.image && step.illustration ? (
                /* Photo + concept diagram as two equal squares, side by side */
                <div className="grid grid-cols-2 gap-3 mb-4">
                  ...
                  <LessonIllustration id={step.illustration} />
                </div>
              ) : step.image ? (
                <img
                  src={step.image}
                  ...
                  style={{ height: 180, objectFit: 'cover', border: '1.5px solid var(--border)', boxShadow: 'var(--shadow-card)' }}
                />
              ) : step.illustration ? (
                <LessonIllustration id={step.illustration} />
              ) : null}
              ...
              <div className="space-y-3">
                {step.options?.map((option, idx) => {
                  ...
                  const feedbackAnim =
                    ...
                    : answered && isSelected ? 'animate-shake'
                    : '';
                  ...
                  return (
                    <button
                      ...
                      disabled={answered}
                      className={`w-full text-left p-4 rounded-2xl text-sm font-medium transition-all active:scale-[0.98] ${feedbackAnim}`}
...
function LessonResult({
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-5 pb-12 text-center" style={{ background: 'var(--bg)' }}>
      ...
      {/* Photo Phil — strikes a bigger pose for a perfect run */}
      <div className="animate-phil mb-2" style={{ opacity: 0 }}>
        <PhotoPhil size={200} pose={quizSteps > 0 && correctCount === quizSteps ? 'starstruck' : 'cheer'} />
      </div>
